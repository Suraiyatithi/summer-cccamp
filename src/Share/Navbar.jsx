...
const Navbar = () => {
  ...
  const navOption = (
    <>
      ...
      <li><Link to="/instructor" onClick={closeMenu}>Instructor</Link></li>
      <li><Link to="/allclass" onClick={closeMenu}>Classes</Link></li>
      <li><Link to="/instructor" onClick={closeMenu}>Popular</Link></li>
      <li><Link to="/offer" onClick={closeMenu}>Offer</Link></li>
      <li><Link to="/blog" onClick={closeMenu}>Blog</Link></li>
      {user && <li><Link to="/dashboard/userhome" onClick={closeMenu}>Dashboard</Link></li>}
      {user ? (
        <>
          <span className="px-2">{user?.displayName}</span>
          <button
            onClick={() => { handleLogOut(); closeMenu(); }}
            className="btn bg-gradient-to-r from-purple-200 via-purple-400 to-purple-800"
          >
            Log out
          </button>
        </>
  ...
  return (
    <>
      ...
      <div
        className={`navbar fixed z-10 shadow-md text-white max-w-screen-xl
        ${menuOpen ? "bg-purple-900" : "bg-opacity-30 bg-black lg:bg-opacity-30 lg:bg-black"}`}
      >
        {/* Navbar Start */}
        <div className="navbar-start">
          <div className="dropdown">
            ...
            {menuOpen && (
              <ul
                tabIndex={0}
                className="menu menu-compact dropdown-content mt-3 p-2 shadow rounded-box w-52 bg-purple-900"
              >
                {navOption}
              </ul>
            )}
